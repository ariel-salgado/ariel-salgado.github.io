<script lang='ts'>
	...
	function remap_value(value: number, start1: number, end1: number, start2: number, end2: number): number {
	...
	function create_circle(): Circle {
		return {
			...
			alpha: 0,
			target_alpha: Number.parseFloat((Math.random() * 0.6 + 0.1).toFixed(1)),
	...
	function animate() {
		...
		circles.forEach((circle, i) => {
			...
			circle.translate_x += (mouse.x / (staticity / circle.magnetism) - circle.translate_x) / ease;
			circle.translate_y += (mouse.y / (staticity / circle.magnetism) - circle.translate_y) / ease;
	...
	function handle_mouse_move(event: MouseEvent) {
		if (canvas_ref) {
			...
			const is_inside = x < w / 2 && x > -w / 2 && y < h / 2 && y > -h / 2;
...
<div bind:this={container_ref} class={cn('size-full', class_name)} aria-hidden='true'>
